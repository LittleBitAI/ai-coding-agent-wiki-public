...
남은 틈. `inject.py` 의 `/busy` 는 데몬을 기다리지 않고 150ms 안에 못 가면 버려진다. 그 턴이 55분을
넘겨 돌면 작업 중에 핑이 들어갈 수 있다. `Stop` 훅이 턴을 되돌려(`declared_continuation.py`) 이어진
작업도 같다. 둘 다 55분 넘는 턴에서만 생긴다.
