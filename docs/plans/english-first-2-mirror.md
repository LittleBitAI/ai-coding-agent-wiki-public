...
| 5 | pages | Git 기준 manifest 확정·32개 산문 영어화·허용 보호 구간 차이 기록 | 완료 — 34장. 설치 안내 2장은 사용자 판단으로 한국어 유지 |
| 6 | lint-index | 영어 줄바꿈 검사·직접 실행 검사·sync와 graph 각각 재생성 | 완료 |
| 7 | status | 두 호스트의 상태·차단·질문 UI 한국어 유지 확인 | 완료 |
| 8 | gate | `--check` 초록 + 표본 역번역 사람 확인 | 완료 — 게이트 초록, 역번역 3건 사람이 읽고 "크게 무너지는 것 없음" |
