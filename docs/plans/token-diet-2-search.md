...
한 번씩으로는 잡음에 묻혔다. 6번은 첫 측정에서도 `scout` 를 부르지 않아 지금과 같은 설정이었는데, 인용이 13 과 8,
입력 토큰이 192K 와 242K 로 갈렸다. 사용자 선택으로 쪽마다 3번씩 더 돌렸다.

#### 판정 — 쪽마다 4번, 중앙값, 2026-09-25

위 두 표의 행(전 1번, `scout` 없는 후 1번)에 전·후 3번씩을 더했다. 오류 0회.

| # | 입력 토큰 전 → 후 | 맞는 인용 전 → 후 | 비용 전 → 후 |
| ---: | ---: | ---: | ---: |
| 1 | 106,411 → 109,740 | 0 → 2 | $0.058 → $0.070 |
| 2 | 241,077 → 219,820 | 2.5 → 3 | $0.136 → $0.137 |
| 3 | 158,776 → 110,054 | 1 → 2 | $0.070 → $0.058 |
| 4 | 133,068 → 133,008 | 0 → 0 | $0.061 → $0.071 |
| 5 | 183,779 → 267,421 | 2 → 3.5 | $0.143 → $0.209 |
| 6 | 297,642 → 220,010 | 9 → 8.5 | $0.218 → $0.182 |
| 7 | 113,992 → 114,461 | 8.5 → 7.5 | $0.094 → $0.093 |
| 8 | 281,341 → 129,686 | 4.5 → 5 | $0.186 → $0.157 |
| 9 | 180,848 → 123,036 | 1.5 → 4 | $0.135 → $0.127 |
| 10 | 281,894 → 155,443 | 0.5 → 4 | $0.130 → $0.152 |
| 중앙값의 합 | 1,978,828 → 1,582,680 (−20%) | 29.5 → 39.5 | $1.23 → $1.26 (+2%) |

평균으로는 턴당 입력 토큰 197,535 → 177,465(−10%), 맞는 인용 3.05 → 3.77, 비용 $0.136 → $0.132, 시간 41 → 38초.
입력 토큰 중앙값이 준 문항은 10개 중 7개다.

완료 기준. 입력 토큰은 줄었고, 파일과 줄이 맞는 인용은 늘었다 — 기계로 볼 수 있는 범위에서 기준을 넘는다. 인용한 줄이
답이 말하는 내용을 실제로 담고 있는지는 사람이 대조하지 않았다. 비용은 그대로다: 줄어든 것이 대부분 값싼 캐시 읽기이고,
첫 측정의 비용 −21% 는 잡음이었다.
