...
| 이미 본 페이지 | 페이지가 "매번 실릴 부분" 을 가진다 — 규칙 문단. 두 번째부터는 그 문단 전문과 경로만 싣는다 (리뷰 1회차 뒤 다시 고름) |
| 중복 제거 대상 | front matter 에 `repeat: rule` 을 선언한 페이지만. 없으면 매번 전문 (리뷰 3회차 뒤 고름) |
| 전문 재전송 | compact 때만 |
...
| 리콜 불변식 | 턴마다 정규식이 고른 페이지 각각이 새 방식의 주입에 글자 그대로 들어 있는가 — `repeat: rule` 을 선언하고 이미 본 페이지는 규칙 문단 전문, 그 밖은 본문 전문. 이름만 보면 강제 조항이 빠져도 녹색이 된다. 하나라도 빠지면 종료 코드 1 |
...
| `txp` | 그 transcript 경로의 sha256 앞 12자. 경로 자체는 적지 않는다 |
...
0. 이번 턴의 `session_id` 가 비어 있으면 중복 제거를 하지 않는다. 호스트가 id 를 안 주면
   `inject.py` 는 `""` 을 적고, `test_trajectory.py` 는 그런 행끼리 이으면 안 된다고 이미 적는다(리뷰 3회차)
1. trajectory 를 한 줄씩 읽되, 이번 세션 id 가 문자열로 들어 있지 않은 줄은 `json.loads` 없이
...
3. compact 감지. 이 세션의 직전 행 `tx` 부터 transcript 끝까지만 읽어서 Claude 의 `"compact_boundary"`
   나 Codex 의 `"type":"compacted"` 가 있는지 본다. 매 턴 새로 붙은 부분만 읽으므로 transcript 가
   수십 MB 여도 작다. 있으면 본 집합을 비우고 이 행에 `reset: true`
   - "직전 행" 은 파일의 마지막 줄이 아니라 이 세션의 마지막 행이다. `trajectory.last_row()` 는 세션을
     안 가리므로 쓰지 않는다. 두 세션이 한 파일에 번갈아 쓰면, 남의 큰 오프셋이 이 세션의 compact 를
     건너뛴다(리뷰 3회차)
   - 그 행의 `txp` 가 이번 경로와 다르거나, 이번 transcript 크기가 그 행의 `tx` 보다 작으면 리셋한다.
     경로가 바뀐 `/clear`·`--resume` 도 이 가드에 걸린다. 같은 id 로 이어지는지는 실물로 확인한다
4. 이번 턴에 정규식이 고른 규칙 중 `repeat: rule` 을 선언했고 본 집합에 있는 것은 전문 대신
   "반복형" 으로 싣는다. `rule_index` 는 그대로 맨 앞이다
5. 기록은 맨 끝이다. 지금 `main` 은 `trajectory.record` 를 부른 뒤에 `additionalContext` 를 만들고
   내보낸다. 그 순서면 본문을 만들다 죽거나 stdout 쓰기가 실패한 턴도 `full` 이 남아, 다음 턴이
   전달되지 않은 페이지를 봤다고 친다(리뷰 3회차). 본문을 다 만들고 `json.dump` 와 flush 가 끝난 뒤에
   기록한다. 기록이 실패하면 `full` 이 없으니 다음 턴에 다시 전문이다

### 어느 페이지가 반복형이 되나 — 페이지가 선언한다

front matter 에 `repeat: rule` 이 있는 페이지만 중복 제거 대상이다. 없으면 오늘처럼 매번 전문이다.
기본값이 안전한 쪽이다(리뷰 3회차, 2026-09-25 사용자 선택).

- 선언하는 조건은 하나다. 매 턴 지켜야 하는 조항이 전부 규칙 문단 안에 있다
- PR ② 는 허브의 주입 대상 페이지를 감사해 조항을 문단 안으로 옮기고 선언을 붙인다. 적어도
  `agent-delegation`(불릿 넷과 셀 배정 표)과 `ask-with-arrow-key-options`(불릿의 추천 순서·기본값 규칙)
- 대상 저장소의 `.wiki` 규칙은 이 PR 에서 손대지 않는다. 예를 들어 ai-nara-shop 의 `project.md` 는
  882턴에 실린 `contract` 인데 "활성 계획과 팀 배정을 먼저 읽어라" 가 `규칙.` 문단 앞에 있다. 그 저장소가
  감사하고 선언을 붙일 때까지 매번 전문이다. 선언을 붙이는 법은 `docs/hooks-setup.md` 에 한 절로 적는다
- 규칙 문단은 두 철자를 다 받는다. `Rule.` 과 `규칙.` — 대상 저장소 페이지는 한국어이고, 번역이 실패하면
  `translate.translate` 가 한국어 원문을 돌려준다. `shrink`·`rule_index` 가 이미 둘 다 받는다(리뷰 3회차)

반복형은 제목, 규칙 문단 전문, 경로다.
...
- `lint` 가 본다. `repeat: rule` 을 선언한 페이지에 규칙 문단(`Rule.` 또는 `규칙.`)이 있고, 길이가
  1,200자 이하인가. 선언하지 않은 페이지에는 아무것도 요구하지 않는다
...
- `session_id` 가 빈 턴 두 개: 둘 다 전문
- 두 세션이 한 trajectory 에 번갈아 쓰고, 한쪽 transcript 에만 compact 가 생긴다: 그쪽만 리셋된다
- `txp` 가 바뀐 턴과 transcript 가 줄어든 턴: 리셋
- 출력 직전에 예외가 난 턴: `full` 이 안 남고 다음 턴에 전문
- `repeat: rule` 이 없는 페이지: 몇 번째 턴이든 전문
...
페이지가 선언을 붙이기 전까지 허브 규칙분만 주니 수치를 적기만 하고 기준에 넣지 않는다. 두 저장소 모두
리콜 불변식 녹색.
...
| 무엇 | 결과 |
| --- | --- |
| Claude 한도 | 글자 단위, 약 10,000자. transcript 의 `UserPromptSubmit` 주입 3,784건 중 전문으로 들어간 가장 큰 것 9,896자(19,466바이트), 파일로 빠진 가장 작은 것 10,015자. 문턱은 9,800바이트 — 글자 수는 바이트 수를 넘지 않는다 |
| 한도를 넘는 턴 | 이 저장소의 평범한 턴(상시 3장)이 10,292바이트라, 설계대로면 Claude 에서 아무 페이지도 "봤다" 가 안 된다(재생 7%, 나라 0%). 사용자 선택으로 한도를 넘을 턴은 `repeat: rule` 페이지를 처음부터 규칙 문단으로 보낸다(`inject.compose`). 넘는 턴의 전문은 2KB 미리보기만 읽혀 원래 안 읽히던 것이다. 리콜 불변식도 그 턴은 규칙 문단을 요구한다 |
| 선언 범위 | 사용자 선택으로 허브 24장 전부. 매 턴 조항을 규칙 문단으로 옮겼다. `run-inside-this-session` 은 `Rule.` 문단이 없어 새로 썼다 |
| 선언 검사 | `lint` 와 `repo_lint` 둘 다 — `wikilib.metadata_errors` 가 본다 |
| Codex 의 `transcript_path` | Codex 0.156 바이너리의 훅 입력 필드 목록에 있다. 실제 세션 행의 `tx` 로 한 번 더 본다 |
| Codex rollout 위치 | 8월부터 이 PC 의 Codex 세션은 Orca 의 홈(`AppData/Roaming/orca/*/home`)에 있다. `replay` 가 거기도 찾는다 |
| Codex 스킬 폴더 | `~/.agents/skills`. `skills/list` 가 기본 홈과 Orca 홈 모두에서 이 폴더를 읽는다. `~/.codex/skills` 는 `CODEX_HOME` 을 따라가 Orca 에서 안 보인다 |
| 호스트 알기 | `hook.py` 가 `--host` 를 넘긴다. 옛 프로젝트별 설치는 안 넘기므로 중복 제거가 꺼진다 |
| 규칙 색인 | 사용자 선택으로 한도를 넘는 턴에만 싣는다. 색인은 2KB 미리보기를 위한 것이고, 한도 안인 턴에서는 규칙 문단 첫 문장의 두 번째 사본이었다(턴당 약 1KB). PR #20 의 "매 턴 색인" 을 이만큼 되돌린다 |
| 게이트 결과 | 이 저장소 세션 누적 2,714KB→1,124KB, 59%(세션 합 중앙값 48%). 나라 16,495KB→9,549KB, 42%. 두 저장소 리콜 불변식 녹색, 이 저장소는 한도 넘는 턴 0. 지연 p95 는 `main` 254ms → 241ms(평범한 턴), 252 → 240(많이 걸림) |

60% 게이트는 1점 모자란다. 색인을 빼기 전에는 57% 였고, 한도 안에 들게 된 턴이 전문을 싣게 되어
(전문 적재 33회 → 102회) 절감 일부가 읽히는 전문으로 바뀌었다. 전문을 덜 싣는 쪽으로 게이트를 맞추는
것은 하지 않았다.
